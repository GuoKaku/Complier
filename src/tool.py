...
class Tool(object):
    def __init__(self):
        ...
        # global information
        self.tool_type_defined = {}
        self.tool_used_memory = {}
        self.tool_functions = {}
        self.tool_global_variables = {}
        self.tool_global_comments = []

        # function information
        self.tool_arguments = {}
        self.tool_start_stack = []
        self.tool_end_stack = []
    ...
    def visit_BinaryOp(self, ast_node, flag=0):
        ...
        if ast_node.OpName == '+':
            ...
            return self.tool_builders.icmp_signed(ast_node.OpName, left, right)
        # elif ast_node.OpName == '===':
        ...
        if ast_node.refType == 'ArrayRef':
            ...
            if ast_node.type == '->':
                ...
                index = ir.Constant(int32,
                                    self.tool_type_defined[name_of_struct.type.pointee.name].index(ast_node.field.name.name))
    ...
    def visit_ContentList(self, ast_node, flag=0):
        arr = []
        if ast_node.listType == 'ExprList':
            for e in ast_node.elements:
                arr.append(self.visit(e, flag))
            return arr
        elif ast_node.listType == 'InitList':
            for e in ast_node.elements:
                arr.append(self.visit(e))
    ...
    def extern_function(self, ast_node):
        ...
        if ast_node.name.name in ['printf', 'scanf']:
            ...
            for i, a in enumerate(arg_eles):
                if i == 0:
                    arguList.append(self.visit(a))
                else:
                    arguList.append(self.visit(a, flag=1))
    ...
    def return_content(self, ast_node, arg=None):
        ...
        if type(ast_node) == SynTree.DeclPointer and arg is not None:
            ...
            type(tmp_node) == SynTree.DeclPointer) or (type(tmp_node) == SynTree.Decl):
                        ...
                        modi_list.append(tmp_node)
                    if type(tmp_node) == SynTree.Identifier:
    ...
    def allocate_indenti(self, ast_node, flag=0, modifier_list=[]):
    ...
    def handle_decl_args(self, ast_node, flag):
        ...
        if flag == 0 and ast_node.init:
            ...
            # self.tool_used_memory: {'a': <ir.AllocaInstr 'a' of type 'i32*', opname 'alloca', operands ()>}：这是一个字典（Dictionary），其中键（Key）是变量名，值（Value）是某个 LLVM IR 中的 alloca 指令对象。alloca 指令用于在函数的栈帧中分配变量的内存空间。
            ...
            # 
            # 
            # ast_node.name:c, self.tool_used_memory:{'a': <ir.AllocaInstr 'a' of type 'i32*', opname 'alloca', operands ()>, 'b': <ir.AllocaInstr 'b' of type 'i32*', opname 'alloca', operands ()>, 'c': <ir.AllocaInstr 'c' of type 'i32*', opname 'alloca', operands ()>}, self.tool_used_memory[ast_node.name]:%"c" = alloca i32
            # 
            # 
            # 
    ...
    def allocate_struct(self, ast_node, flag=0, modifier_list=[]):
        identified_type = self.tool_module.context.get_identified_type(ast_node.name)
        if ast_node.name not in self.tool_type_defined:
            self.tool_type_defined[ast_node.name] = [e.name for e in ast_node.decls]
            tool_type_defined = [self.allocate_variable(e, flag=2) for e in ast_node.decls]
    ...
    def define_struct(self, ast_node, identified_type):
        if ast_node.name not in self.tool_type_defined:
            ...
            for e in ast_node.decls:
                ...
                type(ast_node) == SynTree.DeclPointer) or (type(ast_node) == SynTree.Decl):
                    temp_node = temp_node.type
                    modi_list.append(temp_node)
                if type(temp_node) == SynTree.Identifier:
                    tool_type_defined_list.append(self.allocate_indenti(temp_node, flag=2, modifier_list=modi_list))
                elif type(temp_node) == SynTree.Struct:
                    tool_type_defined_list.append(self.allocate_struct(temp_node, flag=2, modifier_list=modi_list))
                    
                # tool_type_defined_list.append(self.allocate_variable(e, flag=2))
